...
keyMap = { 'w' => north, 'd' => east, 's' => south, 'a' => west }
#remember, up is negative here
$moveMap = { north => [0, -1], east => [1, 0], south => [0, 1], west => [-1, 0] }

$location = [0, 0]
tiles = Hash.new(unknown)
tiles[$location.dup] = start
$minX = 0
$minY = 0
$maxX = 0
$maxY = 0
...
runner.run { |inQ, outQ|
    ...
    #p tiles
    #p $location
    #whew! now return a modified board showing the dragon
    layered = []
    ($minY..$maxY).each { |y|
        row = []
        ($minX..$maxX).each { |x|
...
#send a movement, get back a status
#moves 1 unit unless wall
