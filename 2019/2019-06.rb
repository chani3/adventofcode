...
def findNode(name, branch)
    #p "branch is #{branch}"
    branch.each { |node|
        if node[:name] == name
            return node
        end
        dfs = findNode(name, node[:children])
        if dfs
            return dfs
        end
    }
...
def findOrCreateNode(name, branch)
    node = findNode(name, branch)
    node ||= {name: name, children:[], parent:nil}
    return node
end
...
#build tree
data.each { |line|
    parentName, childName = line.chomp.split(')')
    #p "parent #{parentName} child #{childName}"
    parent = findOrCreateNode(parentName, orphans[:children])
    child = findOrCreateNode(childName, orphans[:children])
    if ! parent[:parent]
        parent[:parent] = orphans
        orphans[:children] << parent
    end
    if child[:parent]
        child[:parent][:children].reject! { |node| node[:name] == childName }
    end
    child[:parent] = parent
