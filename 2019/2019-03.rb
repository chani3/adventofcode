...
def plotWire(wire)
    ...
    step = 0
    #fuckit = 9
    wire.split(',').each { |segment|
        dir = segment.slice!(0)
        move = $moves[dir]
        len = segment.to_i
        #p "pos #{pos} move #{move} len #{len}"
        len.times {
            pos[0] += move[0]
            pos[1] += move[1]
            step += 1
...
intersection = points1.keep_if { |k, v| points2.key?(k) }
p intersection
$sumDistances = []
points1.each_pair { |k, v|
    sum = v + points2[k]
    $sumDistances << sum
}
p $sumDistances
p "--"
p $sumDistances.min
#manhattan = intersection.collect { |point| point[0].abs + point[1].abs }
#p manhattan
#p manhattan.min
