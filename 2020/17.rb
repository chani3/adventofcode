...
def newPocketArray
  Array.new(Width) { Array.new(Height) { Array.new(Depth) { Array.new(Depth, Inactive) }}}
end
...
data.each_with_index { |line, i|
  line.chars.each_with_index { |char, j|
    ...
    pocket[j+Offset][i+Offset][Offset][Offset] = char
...
Directions = [-1, 0, 1]
Neighbours = Directions.product(Directions, Directions, Directions).reject { |a| a == [0,0,0,0] }
def invalid(pos)
  pos[0] < 0 or pos[1] < 0 or pos[2] < 0 or pos[3] < 0 or pos[0] >= Width or pos[1] >= Height or pos[2] >= Depth or pos[3] >= Depth
end
def getState(pos, pocket)
  if invalid(pos)
    return Inactive
  end
  pocket[pos[0]][pos[1]][pos[2]][pos[3]]
end
def getNextState(pos, pocket)
  oldState = getState(pos, pocket)
  myNeighbours = Neighbours.map { |rel| [pos[0]+rel[0],pos[1]+rel[1],pos[2]+rel[2],pos[3]+rel[3]] }
...
6.times do
  ...
  (0..Width-1).each { |x|
    (0..Height-1).each { |y|
      (0..Depth-1).each { |z|
        (0..Depth-1).each { |w|
          nextPocket[x][y][z][w] = getNextState([x,y,z,w], pocket)
        }
      }
    }
  }
